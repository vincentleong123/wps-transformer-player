...
const GAP = { page: 1600, preflight: 900, media: 140 };   // ms per host
...
async function _requestOnce(u, method, headers, kind) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('error', err => { release(); reject(err); });
    req.end();
  });
}
...
function _buildHeaders(source, extra) {
  ...
  const isPage = /text\/html|application\/xhtml/i.test((extra && extra['Accept']) || '');
  const headers = Object.assign({}, extra || {}, {
    'User-Agent': source.ua || CONFIG.userAgent,
    'Accept': isPage ? 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8'
...
module.exports = { init, isSafeUrl, hostAllowed, parseSource, signSource, verifyToken, tokenUrl, preflight, streamProxy, resolveEmbed, rewritePlaylist, tokenizeUri, extractPageMeta, fetchPageText };
